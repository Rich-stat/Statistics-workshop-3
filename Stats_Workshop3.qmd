...
<br> **1. Download the Excel file** [Correlation_2_var_data.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQAht2S0qDi7SIt9B8cipFLvAaidXJJuirncH1llJuqv1Kk?e=4jzuvc) from Canvas and save it to a folder that you will use as your working directory (either in RStudio Cloud or in RStudio on your own laptop).<br>
...
![Uploading a file to RStudio](upload%20corr2.png){.lightbox width="70%"}

- In RStudio, locate the file and set the folder location as your working directory.
:::

<br> **2. Import the file to RStudio and store it as a dataframe called `corr2var`.**<br> The `head()` function shows us the first 6 rows.<br> The `summarise_all(class)` function shows us what type of data is stored in the dataframe. Both are numeric which is what we want.
...
<br> **1. Download the Excel file** [Correlation_3_var_data.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQCtW6wyYpOITIdFFV5qAIrqAc2yGW6Eu4s6K5eKHMynQNs?e=6oweqg) from Canvas and save it to the same folder as above so that it is in your working directory (either in RStudio Cloud or in RStudio on your own laptop).

::: {.callout-note collapse="true" title="Upload excel file to RStudio Cloud" icon="true"}
- Check that the data in the file is 'tidy'
- Upload it to RStudio Cloud, saving it to a folder for this Statistics Workshop 3 session (*NB* If you are using RStudio on your own laptop, simply save the excel file to a relevant folder on your laptop).

![Uploading a file to RStudio](upload%20corr3.png){.lightbox width="70%"}

- In RStudio, locate the file and set the folder location as your working directory.
:::

<br><br> **2. Import the file to RStudio and store it as a dataframe called `corr3var`.**<br> The `head()` function shows us the first 6 rows.<br> The `summarise_all(class)` function shows us what type of data is stored in the dataframe. All three columns contain numeric which is what we want.
...
This shows the distribution of each variable, a scatter plot of each variable against the other and the correlation coefficient. Statistically significant correlation coefficients are indicated with asterisks (\*=significant; \*\*=highly significant; \*\*\*=very highly significant).
:::
...
<br> Produce a histogram to view the standardised residuals visually:
...
<br> Use the Shapiro Wilk test to see if the standardised residuals are normally distributed:
...
<br><br> `b)` Diagnostic plots<br> The following will produce 4 diagnostic plots for the `regr_fit` regression model.
...
1.  Arranges the 4 plots into a 2 x 2 matrix
2.  Produces 4 diagnostic plots from the results stored in the `regr_fit` object

::: {.callout-note collapse="true" title="Click to see the diagnostic plots" icon="false"}
```{r}
#| label: regr.diagn1
#| echo: false
#| eval: true
...
- *Residuals vs. Leverage* - dotted lines = Cook's Distance. If any points are near or cross the dotted lines, they have a strong influence on the model.
:::
...
<br> **1. Download the Excel file** [Mult_Regr_data.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQBxIHBUIsXdSr1w2OHg-9IdAUtxJQKP2ZZMjphLrjjeOK0?e=C6bEXl) from Canvas and save it to a folder that you will use as your working directory (either in RStudio Cloud or in RStudio on your own laptop).

::: {.callout-note collapse="true" title="Upload excel file to RStudio Cloud" icon="true"}
- Check that the data in the file is 'tidy'
- Upload it to RStudio Cloud, saving it to a folder for this Statistics Workshop 3 session (*NB* If you are using RStudio on your own laptop, simply save the excel file to a relevant folder on your laptop).

![Uploading a file to RStudio](upload%20mult%20regr.png){.lightbox width="70%"}
...
**`b)` Diagnostic plots for the best selected model, `Triglycerides = -48.46+1.63(LDL)`**.<br> The results for the selected model has been stored in an object called `model.FINAL`; running the following script will produce 4 diagnostic plots.
...
1.  Arranges the 4 plots into a 2x2 matrix
2.  Creates the four diagnostic plots from the results in the `model.FINAL` object
...
- *Residuals vs. Fitted* - should be even scatter of points around 0. Residuals should have no pattern in relation to predicted (fitted) values. Fitted line should be reasonably close to straight flat line (residuals all about the same = homoscedasticity). Here, some departure, so might not be quite linear.<br> Rows 9, 12 and 17 have the 3 highest residuals.
- *Normal Q-Q* - diag line = normal distribution of residuals. The points should be close to the diagonal line.
- *Scale-Location* - line should be horizontal flat. Similar to Residuals vs Fitted.
- *Residuals vs. Leverage* - dotted lines = Cook's Distance. If any points are near or cross the dotted lines, they have a strong influence.
:::
